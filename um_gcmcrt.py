...
def um_gcmcrt(outname="UM",outdir=None,verbose=True,param_type=None,chem_type="FromOutput",sp_list=None,dumpnum=None,alkali_fix=False,input_dir=None,l_corr_k=True,l_lbl=False,l_rayleigh=True,l_cia=True,l_cloud=False,l_xsec=False,radius=None,R=None):

    if (outdir==None):
        print("An output director (outdir) needs to be specified.")
        exit()
    if (dumpnum==None):
        print("A dump number (dumpnum) needs to be specified.")
        exit()
    ...
    cia_sources = ['H2-H2','He-H2','H2-H','H2-','He-']
    ...
    # gCMCRT tables
    chem_interp_file = '../data/CE_interp/interp_FastChem_1x_cond.txt'
    ...
    if (param_type == "Import"):
        ...
        if (radius is None or R is None):
            print("Error: radius and R both need to be specified.")
            exit()
    ...
    files = [um_dir + "/atmosa.p{}000000{:02}_00".format(x,dumpnum) for x in ["b","c","d","e","f","g"]]
    ...
    pcube = cord.regrid_3d(pcube_old,tcube)
    ucube = cord.regrid_3d(ucube_old,tcube)
    vcube = cord.regrid_3d(vcube_old,tcube)
    ...
    if chem_type == "BS99":
        ...
        sp_stash = []
        for c in chem_cubes:
            if (not str(c.attributes['STASH']).endswith('500')):
                spec_name = STASH[str(c.attributes['STASH'])]
                sp_stash.append(spec_name)
        print("Species in Stash: ",sp_stash)

        if (sp_list is None):
            sp_target = sp_stash
        else:
            sp_target = list(set(sp_stash).intersection(sp_list))

        
        if (alkali_fix):
            # First check that alkali metals aren't in the species list
            if ('Na' in sp_target or 'K' in sp_target):
        ...
        sp,sp_k,sp_ray = reduce_species_list(sp_target)
            ...
            if (not str(c.attributes['STASH']).endswith('500')):
                if (STASH[str(c.attributes['STASH'])] in sp):
                    if (STASH[str(c.attributes['STASH'])] != sp[i]):
                        ...
                        print(STASH[str(c.attributes['STASH'])],sp[i])
                        exit()
                        VMR[:,:,:,i] = c.data[:,:,:]
                    i = i + 1
        if (alkali_fix):
    ...
    elif chem_type == "Table":
        # gCMCRT table
        if (verbose):
            print("Using chemical abundances from gCMCRT tables.\n")
        f = open(chem_interp_file,'r')
        ...
        if (verbose):
            print("Table dimensions: ",nT_tbl,nP_tbl,nL_tbl,nsp_tbl,"\n")

        sp_tbl = f.readline().split()


        if (sp_list is None):
            sp_target = sp_tbl
        else:
            sp_target = list(set(sp_tbl).intersection(sp_list))
            
        # Reduce species list so that we have
        sp,sp_k,sp_ray = reduce_species_list(sp_target)
        if (verbose):
            print("Species in interpolation table: ",sp_tbl,"\n")
            if(sp_list is not None):
                print("Species requested: ",sp_target,"\n")
            print("Species used in gCMCRT opacity calculation: ",sp)
        ...
        data = np.loadtxt(chem_interp_file,skiprows=4)
        mu_1D = data[:,0]
        VMR_1D = np.zeros((nL_tbl,nsp_tbl))
        VMR_1D = np.log10(data[:,1:])

        # Convert 1D to 3D arrays
        mu_tbl = np.zeros((nP_tbl, nT_tbl))
        VMR_tbl = np.zeros((nP_tbl, nT_tbl, nsp_tbl))

        n = 0
        for i in range(nP_tbl):
            for j in range(nT_tbl):
                mu_tbl[i,j] = mu_1D[n]
                VMR_tbl[i,j,:] = VMR_1D[n,:]
                n = n + 1

        # Create a scipy interpolation function for mu
        f_mu = interpolate.interp2d(iT[:], iP[:], mu_tbl, kind='linear')

        #
        # Create a scipy interpolation function for the VMR for each species
        f_VMR = []
        for i in range(nsp_tbl):
            f_VMR.append(interpolate.interp2d(iT[:], iP[:]*1e5, VMR_tbl[:,:,i], kind='linear'))

        if (verbose):
            print("Interpolating VMRs.")
        # Load the array
        for n in range(nsp):
            print("Interpolating ",sp[n])
            # Find the interpolator.
            sp_found=False
            for s in range(0,nsp_tbl):
                if (sp_tbl[s] == sp[n]):
                    idx = s
                    sp_found=True
            if(sp_found == False):
                print("Missing species interpolator.\n")
                exit()
            for j in range(nlat):
                for i in range(nlon):
                    for k in range(nlay):
                        VMR[k,j,i,n] = 10.0**f_VMR[idx](T[k,j,i],P[k,j,i])
        if (verbose):
            print("Done interpolating VMRs.")
